...
- Claude Code, Codex, and OpenCode keep the same `claude` and `gpt` modes. Claude CLI must accept the configured Claude selector, and the active GPT runtime must expose the configured OpenAI model at `high` effort.
