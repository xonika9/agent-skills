...
1. Snapshot candidate body hashes and line-ending/BOM metadata with `python3 scripts/insert_frontmatter.py <root> --inventory-out <inventory.json>`.
...
4. Insert or repair headers only through `python3 scripts/insert_frontmatter.py <root> --manifest <metadata.json> --inventory <inventory.json>`, using an explicit metadata manifest. Never use destructive Git restoration to undo an out-of-scope edit.
